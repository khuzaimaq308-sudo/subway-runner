...
export function AdminPanel() {
  ...
  // Case-insensitive check against all user emails
  ...
  if (!isAdmin) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg,#060614,#100820)", flexDirection: "column", gap: 16, fontFamily: "system-ui", padding: 40 }}>
      ...
      {/* Debug: show detected email so user can verify */}
    ...
    <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, marginTop: 2 }}>
              Subway Runner · Watch Hunt Event · {detectedEmail}
            </div>
          </div>
        </div>
        <a href="/" style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(255,215,0,0.1)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ color: "#FFD700", fontWeight: 700, fontSize: 14 }}>All Players</div>
          <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 11 }}>{users.length} registered</div>
        </div>

        {loading ? (
          <div style={{ padding: 40, textAlign: "center", color: "rgba(255,255,255,0.3)" }}>Loading players…</div>
        ) : users.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", color: "rgba(255,255,255,0.3)" }}>No players yet.</div>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              ...
              <tbody>
                {users.map((u, i) => (
                  <tr key={u.userId}
                    style={{ borderTop: "1px solid rgba(255,255,255,0.04)", cursor: "default" }}
      ...
      <div style={{ marginTop: 20, color: "rgba(255,255,255,0.15)", fontSize: 11 }}>
        More management features coming soon · Admin: {detectedEmail}
      </div>
    </div>
  );
}
